...
export class OrderController {
    static async addOrderItemToOrder(req: AuthRequest, res: Response): Promise<void> {
        ...
        try {
            const product = await productRepository.findOne({ where: {id: productId}, relations: ['store'] });
            if (!product) {
                res.status(404).json({
                    status: 'Error',
            ...
            let order = await orderRepository.findOne({
                where: { user: { id: userId }, status: In([OrderStatus.INIT, OrderStatus.PENDING]) },
    ...
    static async updateOrder(req: AuthRequest, res: Response): Promise<void> {
        ...
        try {
            const order = await orderRepository.findOne({
                where: { id: orderId, user: { id: userId }, status: In([OrderStatus.INIT, OrderStatus.PENDING]) }
            });
