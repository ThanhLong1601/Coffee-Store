...
export class Migration1730364009229 implements MigrationInterface {
    name = 'Migration1730364009229'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            CREATE TABLE \`order_items\` (
                ...
                \`ristretto\` enum ('1', '2') NOT NULL DEFAULT '1',
                \`isOnsite\` tinyint NOT NULL DEFAULT 0,
                \`size\` enum ('250', '350', '450') NOT NULL DEFAULT '350',
                \`time_prepare\` tinyint NOT NULL DEFAULT 0,
                \`prepare_time\` time NULL,
        ...
        await queryRunner.query(`
            ALTER TABLE \`otps\`
            ADD CONSTRAINT \`FK_82b0deb105275568cdcef2823eb\` FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION
        `);
        await queryRunner.query(`
            ALTER TABLE \`orders\`
        ...
        await queryRunner.query(`
            ALTER TABLE \`orders\`
            ADD CONSTRAINT \`FK_0f82354e5b05fd87884eff3a7b5\` FOREIGN KEY (\`storeId\`) REFERENCES \`stores\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION
        `);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            ALTER TABLE \`orders\` DROP FOREIGN KEY \`FK_0f82354e5b05fd87884eff3a7b5\`
        `);
        await queryRunner.query(`
        ...
        `);
        await queryRunner.query(`
            ALTER TABLE \`products\` DROP FOREIGN KEY \`FK_782da5e50e94b763eb63225d69d\`
        `);
        await queryRunner.query(`
            DROP TABLE \`stores\`
        `);
